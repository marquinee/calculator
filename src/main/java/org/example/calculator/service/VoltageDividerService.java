...
public class VoltageDividerService {
    ...
    public static class Result {
        ...
        public Result(String resistors, double vout, double error) {
    ...
    // Стандартные ряды E6, E12, E24, E96 (частично для примера)
    ...
    private static double[] getSeries(String name) {
        return switch (name) {
            ...
            case "E24" -> SERIES[2];
            case "E96" -> SERIES[3];
            default -> SERIES[1]; // E12 по умолчанию
        };
    }
    ...
    public List<Result> calculate(double vin, double voutReq, double tol, String seriesName, int rmin, int rmax) {
        ...
        // Для простоты — только делитель из двух резисторов
        for (int decade = 1; decade <= 1000000; decade *= 10) {
                ...
                for (double r2Base : baseSeries) {
                    ...
                    if (error <= tol) {
                        results.add(new Result(
                                String.format("R1=%.0fΩ, R2=%.0fΩ", r1, r2),
                                vout,
                                error
                        ));
                    }
